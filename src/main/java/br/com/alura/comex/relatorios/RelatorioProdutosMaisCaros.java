...
import br.com.alura.comex.Formatador;

public class RelatorioProdutosMaisCaros extends Relatorios {
    ...
    public void imprimirRelatorio(List<Pedido> pedidos, Consumer<String> impressoraDeRelatorio) {
        if (pedidos == null || pedidos.isEmpty())
            throw new IllegalArgumentException("A lista não deve estar vazia!");
        ...
        produtoMaisCaroCategoria = pedidos.stream()
                ...
                .entrySet().stream().map(entry -> {
                    return new ProdutoMaisCaroCategoria(
                            entry.getKey(), entry.getValue().stream().max(Comparator.comparing(Pedido::getPreco))
                                .stream().map(Pedido::getProduto).findFirst().get(),
                            entry.getValue().stream().max(Comparator.comparing(Pedido::getPreco))
        ...
        produtoMaisCaroCategoria.stream()
                .forEach(produto -> {
                    impressoraDeRelatorio.accept("\n\nCATEGORIA: " + produto.getCategoria()
                            ...
                            + "\nPREÇO: " + Formatador.formatarValor(produto.getPreco()));
                });
    }
